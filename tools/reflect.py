...
class ReflectAction(Action):
    source_function: str = Field(default="", description="待翻译的源函数代码")
...
class ReflectExecutor(ToolExecutor):
    def __call__(self, action, conversation=None):
        ...
        # 返回具体的出错文件、行号和修复建议。
        return ReflectObservation(root_cause=action.error_message[:500], fix_strategy="", content=[])
...
class ReflectTool(ToolDefinition):
    ...
    def create(cls, conv_state=None, **kwargs):
        return [cls(action_type=ReflectAction, observation_type=ReflectObservation, executor=ReflectExecutor())]
